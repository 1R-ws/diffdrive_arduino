...
private:
  ...
  // serial lock (encoder/motor/paper share same serial)
  std::mutex serial_mtx_;

  // paper request publisher
  rclcpp::Node::SharedPtr aux_node_;
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr paper_pub_;

  // polling thread (NOT in 30Hz loop)
  std::atomic<bool> run_paper_{false};
  std::thread paper_thread_;
  double poll_event_period_s_ = 0.2;  // 5 Hz (boleh slowkan)
};
